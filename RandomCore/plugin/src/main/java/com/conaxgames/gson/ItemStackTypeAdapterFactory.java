...
import com.conaxgames.gson.item.EnchantType;
import com.conaxgames.gson.item.ItemMetaType;
import com.conaxgames.gson.item.ItemType;
		...
		return new TypeAdapter<T>() {
			...
			public void write(JsonWriter writer, T item) throws IOException {
				if (item == null) {
					...
					CorePlugin.GSON.toJson(serializeItem((ItemStack) item), ItemType.class, writer);
				}
			}
			...
			public T read(JsonReader reader) throws IOException {
				if (reader.peek() == JsonToken.NULL) {
					...
					return (T) deserializeItem(CorePlugin.GSON.fromJson(reader, ItemType.class));
				}
			}
		};
	}
