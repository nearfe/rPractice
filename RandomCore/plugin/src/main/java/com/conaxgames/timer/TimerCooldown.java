...
import com.conaxgames.timer.event.TimerExpireEvent;
				...
				public void run() {
					if (TimerCooldown.this.timer instanceof PlayerTimer && owner != null) {
						((PlayerTimer) timer).handleExpiry(
								CorePlugin.getInstance().getServer().getPlayer(TimerCooldown.this.owner), TimerCooldown.this.owner);
					}

					CorePlugin.getInstance().getServer().getPluginManager().callEvent(
